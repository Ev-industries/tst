	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
					text = "👑"..member_username.." ["..v.peer_id.."] is Group owner now \n🔖Owners commands : /helpowner"
				else
					text = "👑["..v.peer_id.."] is Group owner now \n🔖Owners commands : /helpowner"
				end
			end
...
local function set_supergroup_photo(msg, success, result)
  ...
  local receiver = get_receiver(msg)
  if success then
    local file = 'core/data/photos/channel_photo_'..msg.to.id..'.jpg'
...
local function run(msg, matches)
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1] == 'gpenable' and not matches[2] then
		...
		if matches[1] == 'gpdisable' and is_admin1(msg) and not matches[2] then
		...
		if matches[1] == "settitle" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == 'setpicture' and is_momod(msg) then
		...
		if matches[1] == 'anti' and is_momod(msg) then
			local target = msg.to.id
		...
		if matches[1] == 'ok' and is_momod(msg) then
			local target = msg.to.id
			     if matches[2] == 'all' then
      	local dsafemode ={
        unlock_group_links(msg, data, target),
		...
		if matches[1] == 'setflood' then
			...
			return 'Floodsensation  has been set to: '..matches[2]
		end
		if matches[1] == 'public' and is_momod(msg) then
			...
			if matches[2] == 'no' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: private")
		...
		if matches[1] == 'forbidlist' and is_momod(msg) then
			local target = msg.to.id
